...
class AdvancedBlockStore:
    ...
    async def update_partial_sigs(self, block_id: str, partial_sigs: Dict[str,str]):
        """
        Можно оставить для совместимости, даже если не используете.
        Просто сохраняем JSON в БД, но по факту уже не вызываем этот метод.
        """
...
class MultiLeaderHotStuffAdvanced:
    def __init__(
        ...
        # REMOVED BLS PARTS: self.threshold_bls = ThresholdBLS(n=self.n, t=self.t)
    ...
    async def _validate_block_data(self, block_data: dict) -> bool:
        ...
        stx_list = block_data.get("stakeTxList")
        if stx_list and isinstance(stx_list, list):
            try:
                from compute_stake_ledger import StakeTransaction
                for item in stx_list:
                    st_obj = StakeTransaction.from_dict(item)
                    if st_obj.amount <= 0:
                        self.logger.warning("[Validate] StakeTxList => amount<=0 => reject")
                        return False
            except Exception as e:
                self.logger.warning(f"[Validate] StakeTxList parse error => {e}")
                return False
        ...
        ds_op = block_data.get("datasetRegistryTx")
        if ds_op and isinstance(ds_op, dict) and self.dataset_registry:
            action = ds_op.get("action")
            dataset_id = ds_op.get("dataset_id")
            if not dataset_id or not action:
                self.logger.warning("[Validate] datasetRegistryTx => missing dataset_id/action => reject")
                return False

            # (A) action == "propose"
            if action == "propose":
                info = await self.dataset_registry.get_dataset_info(dataset_id)
                if info:
                    self.logger.warning(f"[Validate] datasetRegistryTx => propose => ds={dataset_id} already exist => reject")
                    return False
                stake_tekera = ds_op.get("stake_amount", 0.0)
                if stake_tekera <= 0:
                    self.logger.warning("[Validate] propose => stake<=0 => reject")
                    return False
            ...
            # (B) action == "approve"
                ...
                info = await self.dataset_registry.get_dataset_info(dataset_id)
                if not info:
                    self.logger.warning(f"[Validate] approve => ds={dataset_id} not found => reject")
                    return False
                if info["status"] not in ("proposed", "rejected"):
                    self.logger.warning(f"[Validate] approve => ds={dataset_id}, status={info['status']} => reject")
                    return False
            ...
            # (C) action == "slash"
                ...
                info = await self.dataset_registry.get_dataset_info(dataset_id)
                if not info:
                    self.logger.warning(f"[Validate] slash => ds={dataset_id} not found => reject")
                    return False
                if info["status"] == "slashed":
                    self.logger.warning(f"[Validate] slash => ds={dataset_id} => already slashed => reject")
                    return False

            # (D) action == "return_stake"
            elif action == "return_stake":
                info = await self.dataset_registry.get_dataset_info(dataset_id)
                if not info:
                    self.logger.warning(f"[Validate] return_stake => ds={dataset_id} not found => reject")
                    return False
                if info["status"] not in ("approved","proposed"):
                    self.logger.warning(f"[Validate] return_stake => ds={dataset_id}, status={info['status']} => reject")
                    return False

            else:
                self.logger.warning(f"[Validate] datasetRegistryTx => unknown action={action} => reject")
                return False

            self.logger.info(f"[Validate] datasetRegistryTx => action={action}, ds={dataset_id} => pass(dry-run)")
    ...
    async def _apply_block(self, block_id: str, block_data: dict):
        ...
        if stx_list and isinstance(stx_list, list):
            from compute_stake_ledger import StakeTransaction
            tx_objs = [StakeTransaction.from_dict(x) for x in stx_list]
            await self.stake_ledger.run_sealevel_batch(self.sealevel_engine, tx_objs)
            for st_obj in tx_objs:
                self.stake_ledger.mark_stake_tx_confirmed(st_obj.tx_id)

        # 3) TekeraTx (simple transfer)
        tekera_data = block_data.get("tekeraTx")
        if tekera_data and self.cub_tekera:
        ...
        # 5) Транзакции из "tx_list"
        tx_arr = block_data.get("tx_list", [])
        if tx_arr and self.transaction_manager:
            for tx in tx_arr:
                tx_id = tx.get("tx_id")
                await self.transaction_manager.apply_tx(tx)
                if tx_id:
                    self.transaction_manager.mark_tx_confirmed(tx_id)

        # --- теперь Новое: mlRewardTxList ---
        reward_list = block_data.get("mlRewardTxList", [])
        if reward_list and self.cub_tekera:
            for rw_tx in reward_list:
                # Вызываем attempt_ml_reward (проверяет supply, sender="COINBASE"? etc.)
                ok = await self.cub_tekera.attempt_ml_reward(rw_tx, block_data.get("mlProof", {}))
                if not ok:
                    self.logger.warning(f"[MultiAdvHS] _apply_block => mlRewardTxList => fail => {rw_tx}")
                else:
                    # Отмечаем tx подтверждённой
                    c_txid = rw_tx.get("tx_id")
        ...
        if ds_op and isinstance(ds_op, dict) and self.dataset_registry:
            ...
            dataset_id = ds_op.get("dataset_id")
            if action == "propose":
                owner = ds_op.get("owner")
                stake_amt = ds_op.get("stake_amount", 0.0)
                await self.dataset_registry.propose_dataset(dataset_id, owner, stake_amt)
            elif action == "approve":
                await self.dataset_registry.approve_dataset(dataset_id)
            elif action == "slash":
                reason = ds_op.get("reason","not specified")
                await self.dataset_registry.slash_dataset(dataset_id, reason=reason)
            elif action == "return_stake":
                await self.dataset_registry.return_stake(dataset_id)
            else:
