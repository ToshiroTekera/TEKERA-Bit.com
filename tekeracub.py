...
from key import KeyManager  # ваш Schnorr‐KeyManager
...
class CubTekera:
    ...
    async def initialize(self):
        ...
        logging.info(
            f"[CubTekera] node={self.node_id}, address={self.address}, "
            f"local_balance={bal_terabits} Tbits (~{bal_tekera} TEKERA)"
        )
    ...
    async def _load_balance(self) -> int:
        ...
        logging.warning("[CubTekera] _load_balance => record not int => fallback=0.")
        return 0
    ...
    async def load_balance_of(self, address: str) -> int:
        ...
        logging.warning(f"[CubTekera] load_balance_of => can't see other => return 0.")
        return 0
    ...
    async def _increase_plain_balance(self, address: str, add_amt: int):
        ...
        val_obj = LWWValue(new_val, time.time())
        await self.chord_node.replicate_locally(crdt_key, val_obj)
    ...
    async def stake_dataset(self, dataset_id: str, stake_amount_terabit: int) -> bool:
        ...
        async with self._lock:
            ...
            logging.info(f"[CubTekera] stake_dataset => node={self.node_id}, minus {stake_amount_terabit}, newLocal={new_bal}")
    ...
    # ----------------------------------------------------------------
    # propose_transfer_bft
    # ----------------------------------------------------------------
    async def propose_transfer_bft(
        self,
        amount_terabit: int,
        recipient_address: str,
        await_bft: bool = False,
        bft_timeout: float = 30.0
    ):
        ...
        if await_bft:
            ok = await self.transaction_manager.await_bft_confirmation(tx_id, timeout=bft_timeout)
            logging.info(f"[CubTekera] propose_transfer_bft => bft_confirm => success={ok}")
            if not ok:
                logging.warning("[CubTekera] propose_transfer_bft => BFT confirm timeout/fail => no local sub.")
    ...
    # ----------------------------------------------------------------
    # HotStuff DECIDE: _commit_transfer
    # ----------------------------------------------------------------
    async def _commit_transfer(self, tx_data: dict):
        ...
        async with self._lock:
            # Если мы отправитель (не coinbase) - уменьшаем
            if sender_addr == self.address:
                if sender_addr != "COINBASE":
